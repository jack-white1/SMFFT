...
__device__ __inline__ double2 Get_W_value(int N, int m) {
	double2 ctemp;
	sincos(-6.283185308f * __ddiv_rn((double)m, (double)N), &ctemp.y, &ctemp.x);
	return(ctemp);
}
...
__device__ __inline__ void reorder_4_register(double2* A_DFT_value, double2* B_DFT_value, double2* C_DFT_value, double2* D_DFT_value) {
...
__global__ void SMFFT_DIT_external(double2* d_input, double2* d_output) {
	__shared__ double2 s_input[const_params::fft_sm_required];
...
template<class const_params>
__global__ void SMFFT_DIT_multiple(double2* d_input, double2* d_output) {
	__shared__ double2 s_input[const_params::fft_sm_required];
...
int FFT_external_benchmark(double2* d_input, double2* d_output, int FFT_size, int nFFTs, bool inverse, bool reorder, double* FFT_time) {
	GpuTimer timer;
...
int FFT_multiple_benchmark(double2* d_input, double2* d_output, int FFT_size, int nFFTs, bool inverse, bool reorder, double* FFT_time) {
	GpuTimer timer;
	//---------> CUDA block and CUDA grid parameters
...
// ***********************************************************************************
// ***********************************************************************************
int GPU_cuFFT(double2* h_input, double2* h_output, int FFT_size, int nFFTs, bool inverse, int nRuns, double* single_ex_time) {
	...
	size_t total_memory_required_bytes = input_size * sizeof(double2) + output_size * sizeof(double2);
	...
	double2* d_input;
	double2* d_output;
	checkCudaErrors(cudaMalloc((void**)&d_input, sizeof(double2) * input_size));
	checkCudaErrors(cudaMalloc((void**)&d_output, sizeof(double2) * output_size));

	checkCudaErrors(cudaMemcpy(d_input, h_input, input_size * sizeof(double2), cudaMemcpyHostToDevice));
	...
	timer.Start();
	if (inverse) cufftExecZ2Z(plan, (cufftDoubleComplex*)d_input, (cufftDoubleComplex*)d_output, CUFFT_INVERSE);
	else cufftExecZ2Z(plan, (cufftDoubleComplex*)d_input, (cufftDoubleComplex*)d_output, CUFFT_FORWARD);
	timer.Stop();
	...
	*single_ex_time = time_cuFFT;
	...
	//---------> Copy Device -> Host
	checkCudaErrors(cudaMemcpy(h_output, d_output, output_size * sizeof(double2), cudaMemcpyDeviceToHost));
...
int GPU_smFFT_4elements(double2* h_input, double2* h_output, int FFT_size, int nFFTs, bool inverse, bool reorder, int nRuns, double* single_ex_time, double* multi_ex_time) {
	//---------> Initial nVidia stuff
	int devCount;
	size_t free_mem, total_mem;
	checkCudaErrors(cudaGetDeviceCount(&devCount));
...
int Compare_data(double2* cuFFT_result, double2* smFFT_result, int FFT_size, int nFFTs, double* cumulative_error, double* mean_error) {
	double error;
	int nErrors = 0;
	double dtemp;
	int cislo = 0;
...
int main(int argc, char* argv[]) {
	...
	double2* h_input;
	double2* h_output_smFFT;
	double2* h_output_cuFFT;

	if (DEBUG) printf("Host memory allocation...\t");
	h_input = (double2*)malloc(input_size * sizeof(double2));
	h_output_smFFT = (double2*)malloc(output_size * sizeof(double2));
	h_output_cuFFT = (double2*)malloc(output_size * sizeof(double2));
	if (DEBUG) printf("done.\n");

	if (DEBUG) printf("Host memory memset...\t\t");
	memset(h_input, 0.0, input_size * sizeof(double2));
	memset(h_output_smFFT, 0.0, output_size * sizeof(double2));
	memset(h_output_cuFFT, 0.0, output_size * sizeof(double2));
	if (DEBUG) printf("done.\n");
