...
int Compare_data(half2 *cuFFT_result, half2 *smFFT_result, int FFT_size, int nFFTs, double *cumulative_error, double *mean_error){
	double error;
	int nErrors = 0;
	double dtemp;
	int cislo = 0;
	...
	for(int i=0; i<nFFTs; i++){
		for(int f=0;f<FFT_size;f++){
			...
			error_real = get_error(__half2float(cuFFT_result[pos].x),__half2float(smFFT_result[pos].x));
			error_img  = get_error(__half2float(cuFFT_result[pos].y),__half2float(smFFT_result[pos].y));
...
int main(int argc, char* argv[]) {
	...
	if (DEBUG) printf("Host memory allocation...\t");
		h_input        = (half2 *)malloc(input_size*sizeof(float2));
		h_output_smFFT = (half2 *)malloc(output_size*sizeof(float2));
		h_output_cuFFT = (half2 *)malloc(output_size*sizeof(float2));
	if (DEBUG) printf("done.\n");
